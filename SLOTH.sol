...
contract SLOTH is Context, IBEP20, Ownable {
    using Address for address payable;

// Private mappings to keep track of ownership, allowances, and exclusion from fees
...
// Private array of excluded addresses
    address[] private _excluded;

// Booleans to track trading and swapping status
    bool public tradingEnabled;
    bool public swapEnabled;
    bool private swapping;

// Anti-Dump mapping to track the last sell of an address
    mapping(address => uint256) private _lastSell;

// Public variables to store the router and pair addresses
    IRouter public router;
    address public pair;
    uint8 private constant _decimals = 9;
    uint256 private constant MAX = ~uint256(0);

// Total supply of tokens and the maximum value of rTotal
    uint256 private _tTotal = 42e16 * 10**_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

// Minimum amount of tokens required to swap to bnb
    uint256 public swapTokensAtAmount = 42e13 * 10**9;

// Variables to store the genesis block number and deadline for launch tax
    uint256 public genesis_block;
    uint256 private deadline = 1;

// Addresses of the tax, team, other, and burn wallets
    address public taxWallet = 0xbA0883E8C83Ea77646a3A7036bC72b35deF457DC;
    address private otherWallet = 0x648cc2fe186FFA9aDd39EA497038049DE300403c;
    address private teamWallet = 0x648cc2fe186FFA9aDd39EA497038049DE300403c;
    address public burnWallet = 0x000000000000000000000000000000000000dEaD;

// Private constants to store the name and symbol of the token
    ...
    Taxes public taxes = Taxes(3, 5, 0, 0, 0);
    Taxes public sellTaxes = Taxes(3, 5, 0, 0, 0);
    Taxes private launchtax = Taxes(3, 5, 0, 0, 0);
    ...
    function _getValues(
        ...
        to_return = _getTValues(tAmount, takeFee, useLaunchTax);
        (
            to_return.rAmount,
    ...
    function _getTValues(
        ...
        Taxes memory temp;
        if (!useLaunchTax) temp = sellTaxes;
    ...
    function _tokenTransfer(
        ...
        valuesFromGetValues memory s = _getValues(tAmount, takeFee, useLaunchTax);
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap {
        ...
        if (devAmt > 0) {
            payable(teamWallet).sendValue(devAmt);
        }

        uint256 opsAmt = unitBalance * 2 * temp.ops;
        if (opsAmt > 0) {
            payable(otherWallet).sendValue(opsAmt);
        }
    }
    ...
    function updateotherWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0),"Fee Address cannot be zero address");
